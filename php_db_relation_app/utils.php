...
function modifier_produit($id,$new_libelle,$new_prix,$new_photo )
{
	$cnx=connecter_db();
	$rp=$cnx->prepare("update produit set libelle = ? , prix= ? , photo=?  where id=?");
	$rp->execute(array($new_libelle,$new_prix,$new_photo,$id));
}
//modification de produit
